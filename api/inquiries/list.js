 ...
 * R4-1 Phase 4 — Returns the inquiries index (sorted newest-first, max 500).
 *
 * Returns: { ok, count, inquiries: [{threadId, from, subject, customer_name,
 *             event_date, guest_count, status, updated_at}] }
 */
...
module.exports = async (req, res) => {
  ...
  let raw;
  try { raw = await kvGet(INDEX_KEY); } catch (e) {
    return res.status(500).json({ error: 'KV read failed', detail: e.message });
  }

  let index = raw ? (typeof raw === 'string' ? JSON.parse(raw) : raw) : [];
  if (!Array.isArray(index)) index = [];

  return res.status(200).json({ ok: true, count: index.length, inquiries: index });
};
