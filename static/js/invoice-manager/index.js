...
(function () {
  ...
  function renderPanel(root, data) {
    ...
    var rangeBtns = RANGE_LABELS.map(function (r) {
      return '<button class="sales-range-btn' + (r.key === _range ? ' active' : '') + '" ' +
        ...
        r.key !== 'custom' ? r.label : r.label +
        '</button>';
    }).join('');
    // Rebuild properly to avoid ternary string concat issues
    rangeBtns = RANGE_LABELS.map(function (r) {
      return '<button class="sales-range-btn' + (r.key === _range ? ' active' : '') +
        '" data-range="' + r.key + '" onclick="window.invoiceMgr._setRange(\'' + r.key + '\')">' +
        r.label + '</button>';
    }).join('');
    ...
    var tiles = [
      ...
      return '<div class="sales-stat-card">' +
        ...
        '<div class="sales-stat-val ' + t.color + '" >' + (isEmpty && t.label !== 'Charges $' && t.label !== 'Paid $' ? '$—' : t.val) + '</div>' +
  ...
  /* ── Refresh: fetch + re-render tiles only ────────────────────────────── */
  function refresh() {
