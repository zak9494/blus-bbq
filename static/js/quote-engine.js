...
/* ── Constants ──────────────────────────────────────────────────────────── */
window.QB_DELIVERY_FEE = 50;      // flat delivery fee, dollars
window.SALES_TAX_RATE  = 0.0825;  // Texas sales tax (8.25%)
 ...
 */
window.calcQuoteTotals = function calcQuoteTotals(foodSubtotal, servicePct, deliveryFee, taxExempt) {
  const serviceCharge = Math.round(foodSubtotal * ((servicePct || 0) / 100) * 100) / 100;
  const baseTax       = Math.round(foodSubtotal * window.SALES_TAX_RATE * 100) / 100;
